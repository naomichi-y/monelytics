...
- **ドロップダウンの開閉トグルは `<a>` だが `role="button"` を持つ。**
  Bootstrap 5 の作法で、`getByRole('link')` では引けない。
- **`smoke.spec.js` は全画面を一巡する。** 版を上げたときに出やすいのは
  「特定の画面だけ JS が落ちる」「差し替え忘れた資産が 404 になる」で、
  個別のテストより巡回のほうが早く気付ける。
