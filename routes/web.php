...
// どのルートにも当たらない URL も web グループに通す。
//
// ルートに当たらないと 404 はミドルウェアを通る前に描かれ、セッションが
// 始まっていない。レイアウトのナビバーは Auth::check() で出し分けるので、
// ログイン済みでも「会員登録 / ログイン」が出ていた。abort(404) を返すだけの
// ルートを最後に置けば、同じ 404 ページがセッション付きで描かれる。
//
// fallback は GET にしか載らない。未知の URL への POST は、以前の 404 では
// なく 405 になる。
